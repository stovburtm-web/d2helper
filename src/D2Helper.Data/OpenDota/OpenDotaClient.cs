...
public sealed class OpenDotaClient
{
    ...
    public async Task<PlayerProfile?> GetPlayerAsync(long steamId64, CancellationToken ct = default)
    {
        ...
        using var resp = await _http.GetAsync($"players/{accountId}", ct).ConfigureAwait(false);
    ...
    {
        using var resp = await _http.GetAsync($"players/{accountId}/wl", ct).ConfigureAwait(false);
    ...
    private async Task<IReadOnlyDictionary<int, string>> GetHeroNamesAsync(CancellationToken ct)
    {
        ...
        using var resp = await _http.GetAsync("heroes", ct).ConfigureAwait(false);
